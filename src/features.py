...
def extract_features_from_fits(fits_path):
    ...
    t0 = time[mask0]
    f0 = flux[mask0]
    if t0.size < MIN_LENGTH or f0.size < 101:
        return None
    detr = f0 - savgol_filter(f0, 101, 3)
    flux_norm = MinMaxScaler().fit_transform(detr.reshape(-1, 1)).flatten()
    mask1 = np.isfinite(flux_norm)
    t = t0[mask1]
    f = flux_norm[mask1]
    if t.size < MIN_LENGTH or f.size < MIN_LENGTH:
        return None
    std_f, skew_f, kurt_f, q25, q50, q75 = basic_stats(f)
    pratio, ten, cent = fft_features(f)
    ls_peak_power, rot_period = lomb_scargle_features(t, f)
    ac1, ac2, ac3 = autocorr_features(f)
    cwt1, cwt2, cwt3 = cwt_features(f)
    period_bl, depth_bl, dur_bl, snr_bl, t0_bl = bls_features(t, f)
